...
from (select order_id,
      ...
      from (select order_id,
           ...
           (select id org_id,
                   ...
                   region_id
            from tms_dim.dim_organ_full
            where dt = '2025-07-18') organ
           on detail.sender_district_id = organ.region_id
               left join
           (select id,
                   parent_id
            from tms_dim.dim_region_full
            where dt = '2025-07-18') district
...
from (select id,
     ...
     (select id,
      ...
      where dt = '2025-07-18') truck_info
...
from (select org_id,
     ...
     (select id,
             ...
             parent_id
      from tms_dim.dim_region_full
      where dt = '2025-07-18') city_for_level1
     ...
     (select id,
             ...
             parent_id
      from tms_dim.dim_region_full
      where dt = '2025-07-18') province_for_level1
     ...
     (select id,
             ...
             parent_id
      from tms_dim.dim_region_full
      where dt = '2025-07-18') province_for_level2
...
where dt >= date_add('2022-07-11', -recent_days + 1)
group by org_id,
         org_name,
         city_id,
         city_name,
...
from (select recent_days,
     ...
     (select id,
      ...
      where dt = '2025-07-18') for_line_name
...
from tms_dws.dws_trans_org_deliver_suc_1d lateral view
         explode(array(7, 30)) tmp as recent_days
where dt >= date_add('2025-07-18', -recent_days + 1)
group by org_id,
         org_name,
         city_id,
         city_name,
